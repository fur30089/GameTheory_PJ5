...
int shell(int argc, const char* argv[]) {
	...
	for (int i = 1; i < argc; i++) {
      ...
      std::shared_ptr<agent> play(new weight_agent(para.substr(para.find("=") + 1)));
   ...
   */  
        ...
        if (move == "?") {
        	...
        	if (a.type() == action::place::type) {  //evil
            host.at(id).apply_action(a,-1);
        		//int hint = 1;
            rndenv evil;
            ...
            host.at(id).apply_action(a,hint); // you should pass the hint tile to your player
        	}
         else{
         int way=a.event() & 0b11;  //player
         host.at(id).apply_action(a,way); // you should pass the hint tile to your player
         }
