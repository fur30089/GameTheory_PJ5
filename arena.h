...
public:
	...
	public:
		match(const std::string& id, std::shared_ptr<agent> play, std::shared_ptr<agent> evil) : id(id), play(play), evil(evil) {val = -1; } //val = -1
		std::string name() const { return id; }

		action take_action() {
			agent& who = take_turns(*play, *evil);
			return who.take_action(state(), val);
		}
  	bool apply_action(action move, int v) { //v代表way,hint  注意前九步
       bool rv = episode::apply_action(move);
       if (rv) val = v;
       return rv;
  	}
